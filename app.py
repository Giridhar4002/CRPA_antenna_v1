...
def _fig_polar(az_t, db_t, steer_az, null_t, depths_t, dyn):
    ...
    _fig_dark(fig); ax.set_facecolor(PANEL_BG)
    th = np.radians(az_t); r = np.clip(np.array(db_t)+dyn, 0, None)
    ...
    ax.tick_params(colors="white"); ax.grid(color=GRID_COL, alpha=0.5)
...
def _fig_cart(az_t, db_t, steer_az, null_t, depths_t, dyn):
    ...
    _fig_dark(fig); _ax_dark(ax)
    azs = np.array(az_t); dbs = np.array(db_t)
    ...
    ax.set_ylim([-dyn-2, 3]); ax.set_xlim([-180,180])
...
def _fig_el(el_t, db_t, steer_az, steer_el, null_t, dyn):
    ...
    ax.set_ylim([-dyn-2,3]); ax.set_xlim([0,90])
...
def _fig_layout(pos_t, wl, steer_az, null_t, wm_t, wp_t, max_w, radius):
    pos = np.array(pos_t); pos_cm = pos*100; wl_cm = wl*100
    patch = 0.5*wl_cm*0.85; cmap = plt.cm.plasma
    fig, ax = plt.subplots(figsize=(5.2,5.2))
    _fig_dark(fig); ax.set_facecolor(PANEL_BG); ax.set_aspect("equal")
    ...
    for i,(naz,_) in enumerate(null_t):
        c=NULL_COLS[i%4]; nr=np.radians(naz)
    ...
    lim=R*1.35; ax.set_xlim([-lim,lim]); ax.set_ylim([-lim,lim])
...
def _fig_phasors(wr_t, wi_t, max_w, N_total):
    ...
    mags = np.abs(w); phases = np.angle(w, deg=True)
    fig, ax = plt.subplots(figsize=(5.2,5.2))
    _fig_dark(fig); ax.set_facecolor(PANEL_BG)
...
def _fig_3d(pos_t, wr_t, wi_t, wl, eff, dyn):
    ...
    POW = np.array(pow_t); peak = max(np.max(POW), 1e-30)
    DB  = to_db(POW, peak, -dyn)
    R   = np.clip(DB+dyn, 0, None)/dyn
    az_v = np.array(az_t); el_v = np.array(el_t)
    AZ_r = np.radians(np.meshgrid(az_v, el_v)[0])
    EL_r = np.radians(np.meshgrid(az_v, el_v)[1])
    X=R*np.cos(EL_r)*np.sin(AZ_r); Y=R*np.cos(EL_r)*np.cos(AZ_r); Z=R*np.sin(EL_r)
    fig = plt.figure(figsize=(7.5, 5.5)); _fig_dark(fig)
    ax  = fig.add_subplot(111, projection="3d"); ax.set_facecolor(PANEL_BG)
    ...
    for p in [ax.xaxis, ax.yaxis, ax.zaxis]:
        p.pane.fill=False; p.pane.set_edgecolor("#333")
    fig.tight_layout(); return fig
...
peak_gain_dbi = 10*np.log10(N_total) + 10*np.log10(max(0.01*patch_eff*4*math.pi*0.25, 1e-10)) + 5.0
